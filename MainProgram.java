...
/**  Main program for HybridHashTable 
 * Adapted from Dr. John Ramirez's CS 1501
 */
public class MainProgram
{
	public static void main(String [] args)
	{
		...
		// table.
		HybridSymbolTable<Integer> A = new HybridSymbolTable<Integer>(15);

		// Put some data into the HybridSymbolTable.  This should hash the data using linear
		...
		System.out.println("\tAdding some data to the HybridSymbolTable");
		A.put("Zany", 20);
		A.put("Wacky", 100);
		A.put("Hilarious", 40);
		A.put("Fun", 50);
		...
		// class. This allows it to be nested within HybridSymbolTable but it can still be publicly
		...
		// that it allows the user to extract the (key, value) pairs from the HybridSymbolTable
		// in a clearly defined way that still encapsulates the type within HybridSymbolTable.
		...
		// will again go through the (key, value) pairs of the HybridSymbolTable.
		System.out.println("\tShowing contents using each() method:");
		HybridSymbolTable.Pair<Integer> curr;
		...
		// Accessing the HybridSymbolTable using a for loop.  Note that the items that used int
		...
		// the HybridSymbolTable is not found.
		...
		HybridSymbolTable<String> B = new HybridSymbolTable<String>(4);
		...
		HybridSymbolTable.Pair<String> currB;
		...
		// Now make another HybridSymbolTable, this time of StringBuilder, which is mutable but
		// not Comparable
		HybridSymbolTable<StringBuilder> C = new HybridSymbolTable<StringBuilder>(4);
		B.reset();
		while ((currB = B.each()) != null)
		{
		...
		System.out.println("HybridSymbolTable of <String, StringBuilder>");
		HybridSymbolTable.Pair<StringBuilder> currC;
		while ((currC = C.each()) != null)
		{
			System.out.println("Key: " + currC.key + " Value: " + currC.value);
		}
		System.out.println();
		System.out.println("Trying to sort HybridSymbolTable of StringBuilder");
		try
		{
			C.sort();
		}
		...
		// redefining as long as you define the Iterator for HybridSymbolTable correctly.  This is
	...
	// the values within the HybridSymbolTable.  See the required class header for HybridSymbolTable
	...
	public static <V> void showData(HybridSymbolTable<V> A)
	{
		for (V X: A)
		{
	...
	// of the original array in a new HybridSymbolTable.  Note that this is NOT a mutator.
	// Since the key for any HybridSymbolTable object must be a String, the array_flip() method
	// will only work if value of the original HybridSymbolTable is also a String.  If the value
	...
	public static <V> void testFlip(HybridSymbolTable<V> Ar)
	{
		try
		{
			HybridSymbolTable<String> Aflip = Ar.array_flip();
			HybridSymbolTable.Pair<String> currA;
